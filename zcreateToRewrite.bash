...
Usage:	zcreate
	...
	[-C] Device class. Only use this type of device. 
		Default: Use all drives
		Options: "hdd","ssd"
...
getbays() { # Fills array BAYS with physical slot number, whether there is a drive present or not (creates array of all drive slots, wheter a is drive present or not)
	i=0
	j=3
	BAYS=$((cat /etc/vdev_id.conf | awk "NR>2" | wc -l) 2>/dev/null)
	while [ $i -lt $BAYS ];do
...
getdrives() { # For each DRIVE in BAY, check if it is a block device. If true add to DRIVES array therefore excludvi ing any empty slots. If Device class specified only use that type of device
	DRIVES=()
    ...
    elif [ $DEVICE_CLASS == "hdd" ];then
        for DRIVE in ${BAY[*]};do
		    if [ -b /dev/disk/by-vdev/$DRIVE ];then
                if [ $(cat /sys/block/$(basename $(readlink /dev/disk/by-vdev/$DRIVE))/queue/rotational) -eq 1 ];then
			        DRIVES+=( "$DRIVE" )
                fi
		    fi
	    done
    elif [ $DEVICE_CLASS == "ssd" ];then
        for DRIVE in ${BAY[*]};do
		    if [ -b /dev/disk/by-vdev/$DRIVE ];then
                if [ $(cat /sys/block/$(basename $(readlink /dev/disk/by-vdev/$DRIVE))/queue/rotational) -eq 0 ];then
			        DRIVES+=( "$DRIVE" )
                fi
		    fi
	    done
...
while getopts 'a:bC:cd:Dfm:n:l:tqv:zh' OPTION; do
	case ${OPTION} in
	a)
		ASHIFT_FLAG=yes
		ASHIFT=${OPTARG}
    ...
    C)
        DEVICE_CLASS=${OPTARG}
        if [ $DEVICE_CLASS != "ssd" ] && [ $DEVICE_CLASS != "hdd" ];then
            echo "device class must be either 'ssd' or 'hdd'"
            exit 0
        fi
        ;;
	c)
...
#		/opt/tools/bin/tunepool $ZPOOL_NAME $QUIET
#	fi
